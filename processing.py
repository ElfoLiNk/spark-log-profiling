    ...
    for log in glob.glob("./logs/app-*"):
            ...
            for line in logfile:
                ...
                try:
                    ...
                    if data["Event"] == "SparkListenerStageCompleted":
                        # print(data)
                        stage_dict[data["Stage Info"]["Stage ID"]]["numtask"] = data["Stage Info"]['Number of Tasks']
                        for acc in data["Stage Info"]["Accumulables"]:
                            if acc["Name"] == "internal.metrics.executorRunTime":
                                stage_dict[data["Stage Info"]["Stage ID"]]["duration"] = int(acc["Value"])
                                stage_dict[0]["totalduration"] += int(acc["Value"])
                            if acc["Name"] == "internal.metrics.input.recordsRead":
                                stage_dict[data["Stage Info"]["Stage ID"]]["recordsread"] = acc["Value"]
                            if acc["Name"] == "internal.metrics.shuffle.read.recordsRead":
                                stage_dict[data["Stage Info"]["Stage ID"]]["shufflerecordsread"] = acc["Value"]
                            if acc["Name"] == "internal.metrics.output.recordsWrite":
                                stage_dict[data["Stage Info"]["Stage ID"]]["recordswrite"] = acc["Value"]
                            if acc["Name"] == "internal.metrics.shuffle.write.recordsWritten":
                                stage_dict[data["Stage Info"]["Stage ID"]]["shufflerecordswrite"] = acc["Value"]
                except KeyError:
                    print(data)
        ...
        for skipped_id in skipped:
            for stage_id1 in stage_dict.keys():
                if stage_id1 != skipped_id and stage_dict[skipped_id]["RDDIds"] == stage_dict[stage_id1]["RDDIds"]:
        ...
        for stage_id in sorted(stage_dict.keys()):
            ...
            if stage_id not in skipped:
                ...
                if parent_output != 0:
                    stage_dict[stage_id]["nominalrate"] = parent_output / (stage_dict[stage_id]["duration"] / 1000.0)
                elif parent_input != 0:
                    stage_dict[stage_id]["nominalrate"] = parent_input / (stage_dict[stage_id]["duration"] / 1000.0)
                else:
                    stage_input = stage_dict[stage_id]["recordsread"] + stage_dict[stage_id]["shufflerecordsread"]
                    if stage_input != 0 and stage_input != stage_dict[stage_id]["numtask"]:
                        stage_dict[stage_id]["nominalrate"] = stage_input / (stage_dict[stage_id]["duration"] / 1000.0)
                    else:
        ...
        for key in stage_dict.keys():
            if key not in skipped:
                ...
                stage_dict[key]["weight"] = np.mean([old_weight, totalduration / stage_dict[key]["duration"]])
                totalduration -= stage_dict[key]["duration"]

        # Create json output
        with open("./json/" + re.sub("[^a-zA-Z0-9.-]", "_", app_name) + ".json", "w") as jsonoutput:
